...
def extract(loader):
    ...
    for i, (imgs, lbls) in enumerate(tqdm(loader)):
        ...
        # Average the 5 crops
        cls = cls.view(bs, ncrops, -1).mean(dim=1)
...
if __name__ == "__main__":
    print("Extracting train (TTA)...")
    train_feats, train_labels = extract(train_loader)

    print("Extracting test (TTA)...")
    ...
    torch.save(
        ...
        "features/cifar10_train_tta.pt"
    )

    torch.save(
        {"features": test_feats, "labels": test_labels},
        "features/cifar10_test_tta.pt"
    )
