...
if __name__ == "__main__":
    ...
    train = torch.load("features/cifar10_train_tta.pt")
    test = torch.load("features/cifar10_test_tta.pt")
...
import matplotlib.pyplot as plt
    ...
    # Save to a separate plot for TTA
    plt.figure(figsize=(8,6))
    Ns, accs = zip(*results)
    
    plt.plot(Ns, accs, marker='o', linestyle='-', linewidth=2, markersize=8, color='green')
    
    plt.xscale('log')
    plt.xticks(Ns, Ns)
    plt.grid(True, which='both', linestyle='--', alpha=0.7)
    
    plt.title('Few-Shot KNN Accuracy (DINOv2 TTA 5-Crop)', fontsize=14)
    plt.xlabel('Number of Shots per Class (N)', fontsize=12)
    plt.ylabel('Accuracy', fontsize=12)
    
    plt.tight_layout()
    plt.savefig('results_tta.png', dpi=300)
    print("Saved plot to results_tta.png")
